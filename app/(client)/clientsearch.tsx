...
export default function SignupPageScreen() {
  ...
  const {
    handleSubmit: reserver,
    formState: { errors: reserver_errors },
    control: reserver_control,
    setValue: setValueReserver,
  } = useForm<ReservationPlacingType>({
    resolver: yupResolver(ReservationPlacingValidation),
  });
  const { refetch } = useGetAllReservation();
  const { mutateAsync: createReservation } = usePostReservation({
    action() {
      refetch();
    },
  });
  ...
  return (
    <ScrollView style={stylesPerso.container}>
      <View style={page.paddingnormal}>
        <Marginer value={15} />
        <Text style={styles.font}>RECHERCHER UN TRAJET</Text>
        <Marginer value={15} />
        <View>
          ...
          />
          {errors.depart && (
            <Text style={styles.errors}>{errors.depart.message}</Text>
          )}
          <Marginer value={5} />
          ...
          />
          {errors.arrive && (
            <Text style={styles.errors}>{errors.arrive.message}</Text>
          )}
          <Marginer value={5} />
          <Text>Date : </Text>
          ...
          {show && <Calendar onDayPress={handleDayPress} style={styles.cal} />}
          {errors.date && (
            <Text style={styles.errors}>{errors.date.message}</Text>
          )}
          <Button title="Rechercher" onPress={submit(searchTrajet)} />
        </View>
        <Marginer value={50} />
        {isPending && (
          <View>
        ...
        {trajets &&
          trajets?.data.map((trajet: any, index: any) => (
      ...
      <Modal visible={showpanier} animationType="slide">
        <ScrollView style={page.paddingnormal}>
          ...
          <View style={trajetbox.item}>
            ...
            <View style={styles.flexy}>
              <Text style={trajetbox.itemheure}>
                {" "}
                {selectedTrajet?.heure_depart}
              </Text>
              <Text style={trajetbox.itemduree}>
                ----- {selectedTrajet?.duree_trajet} ----
              </Text>
              <Text style={trajetbox.itemheure}>
                {" "}
                {selectedTrajet?.heure_arrive}
              </Text>
            </View>
            <View style={styles.flexy}>
              <Text style={trajetbox.itemville}>
                {" "}
                {selectedTrajet?.ville_depart}
              </Text>
              <Text style={trajetbox.itemville}>
                {" "}
                {selectedTrajet?.ville_arrive}
              </Text>
            </View>
            <Text style={trajetbox.itemtext}>
          ...
          />
          {reserver_errors.nbr_place && (
            <Text style={styles.errors}>
              {reserver_errors.nbr_place.message}
            </Text>
          )}
