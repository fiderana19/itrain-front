...
import {
  ...
  EditTrainValidation,
} from "@/validation/train.validation";
...
export default function HomeScreen() {
  ...
  const { mutateAsync: trainCreate } = usePostTrain({
    action() {
      refetch();
    },
  });
  const { mutateAsync: trainEdit } = useEditTrain({
    action() {
      refetch();
    },
  });
  const { mutateAsync: trainDelete } = useDeleteTrain({
    action() {
      refetch();
    },
  });
  ...
  const {
    ...
    reset,
  } = useForm<EditTrainType>({
    resolver: yupResolver(EditTrainValidation),
  });
  const [selectedTrain, setSelectedTrain] = useState<EditTrainType | null>(
    null,
  );
    ...
    <ScrollView style={stylesPerso.container}>
      <View style={page.paddingnormal}>
        ...
        <View style={styles.acc}>
          <Text style={styles.title}>LISTE DES TRAINS</Text>
          <Marginer value={15} />
      ...
      <Modal animationType="slide" visible={addvisible}>
        <ScrollView style={styles.comptepagescroll}>
          ...
          <Text style={styles.font}>AJOUTER UN TRAIN</Text>
          <Marginer value={5} />
          <View>
            ...
            />
            {create_errors.numero_train && (
              <Text style={styles.errors}>
                {create_errors.numero_train.message}
              </Text>
            )}
            <Text>Capacité : </Text>
            <Controller
            ...
            />
            {create_errors.capacite && (
              <Text style={styles.errors}>
                {create_errors.capacite.message}
              </Text>
            )}
            <Text>Classe : </Text>
            <Controller
            ...
            />
            {create_errors.classe && (
              <Text style={styles.errors}>{create_errors.classe.message}</Text>
            )}
            <Button title="Ajouter" onPress={submitCreate(createTrain)} />
          </View>
        </ScrollView>
      </Modal>
      ...
      <Modal animationType="slide" visible={editvisible}>
        <ScrollView style={styles.comptepagescroll}>
          ...
          <Text style={styles.font}>MODIFIER TRAIN</Text>
          <Marginer value={5} />
          {selectedTrain && (
            <View>
              <Text>Numero du train : </Text>
              ...
              />
              {edit_errors.numero_train && (
                <Text style={styles.errors}>
                  {edit_errors.numero_train.message}
                </Text>
              )}
              <Text>Capacité : </Text>
              <Controller
              ...
              />
              {edit_errors.capacite && (
                <Text style={styles.errors}>
                  {edit_errors.capacite.message}
                </Text>
              )}
              <Text>Classe : </Text>
              <Controller
              ...
              />
              {edit_errors.classe && (
                <Text style={styles.errors}>{edit_errors.classe.message}</Text>
              )}
              <Button title="Modifier" onPress={submitEdit(editTrain)} />
            </View>
          )}
        </ScrollView>
      </Modal>
