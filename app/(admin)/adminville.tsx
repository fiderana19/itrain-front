...
export default function HomeScreen() {
  ...
  const { mutateAsync: villeCreate } = usePostVille({action() {
    refetch();
  },});
  const { mutateAsync: villeEdit } = useEditVile({action() {
    refetch();
  },});
  const { mutateAsync: villeDelete } = useDeleteVille({action() {
    refetch();
  },})
  const { handleSubmit: submitCreate, formState: { errors: create_errors }, control: create_control, setValue: setAddValue } = useForm<CreateVilleType>({
    resolver: yupResolver(VilleValidation)
  })
  const { handleSubmit: submitEdit, formState: { errors: edit_errors }, control: edit_control, setValue: setEditValue, reset } = useForm<EditVilleType>({
  })
  const [selectedVille, setSelectedVille] = useState<EditVilleType | null>(null);
  ...
  return (
      <ScrollView style={stylesPerso.container}>
        <View style={page.paddingnormal}>
          ...
          { villes && villes.map((ville: any, index: any) => (
          <View key={index} style={item.item}>
            <Image source={require(Bg)} style={item.bg} />
            <View style={item.descri}> 
              <Text style={trajetbox.itembillet}> { ville?.nom_ville } { ville?.code_ville } </Text>
              <View style={styles.flexend}>
                <Pressable onPress={()=>{handleEdit(ville)}}>
                  <View style={stylesPerso.btnPrimary}>
                    <Ionicons name='pencil-outline' style={stylesPerso.iconfont} />
                    <Text>Modifier</Text>
                  </View>
                </Pressable>
                <Pressable onPress={()=>{deleteHandler(ville?.code_ville)}}>
          ...
          <Modal animationType='slide' visible={addvisible}>
            ...
            <Pressable onPress={()=>{setAddVisible(false)}} style={styles.closeview}>
              ...
              <View>
                ...
                />
                {create_errors.code_ville && <Text style={styles.errors}>{create_errors.code_ville.message}</Text>}
                <Text>Nom : </Text>
                <Controller 
                  name="nom_ville"
                  control={create_control}
                ...
                />
                {create_errors.nom_ville && <Text style={styles.errors}>{create_errors.nom_ville.message}</Text>}
                <Button
                title="Ajouter"
                onPress={submitCreate(createVille)}
                />
              </View>
            </ScrollView>
          </Modal>
        ...
        <Modal animationType='slide' visible={editvisible}>
          ...
          <Pressable onPress={()=>{setEditVisible(false)}} style={styles.closeview}>
            ...
            <View>
              ...
              />
              {edit_errors.code_ville && <Text style={styles.errors}>{edit_errors.code_ville.message}</Text>}
              <Text>Nom : </Text>
              <Controller 
                name="nom_ville"
                control={edit_control}
              ...
              />
              {edit_errors.nom_ville && <Text style={styles.errors}>{edit_errors.nom_ville.message}</Text>}
              <Button
                title="Modifier"
                onPress={submitEdit(editVille)}
              />
            </View>
            }
          </ScrollView>
